...
public class ProductServiceImpl implements ProductService {
	...
	@Autowired
	public ProductServiceImpl(ProductRepository productRepository, FinanceCompanyRepository financeCompanyRepository) {
	...
	public List<Product> mergeToSavings(List<Saving> savings) {
		return savings.stream().map(mapper -> {
			return productRepository.findByCodeAndCompanyNumberAndProductType(mapper.getFinanceProductCode(), mapper.getFinanceCompanyNumber(), ProductType.INSTALLMENT_SAVING)
					.map(product -> {
						...
						product.setTestState(TestState.DONE);
						return productRepository.save(product);
					}).orElseGet(() -> {
	...
	public List<Product> mergeToDeposits(List<Deposit> deposits) {
		return deposits.stream().map(mapper -> {
			return productRepository.findByCodeAndCompanyNumberAndProductType(mapper.getFinanceProductCode(), mapper.getFinanceCompanyNumber(), ProductType.FIXED_DEPOSIT)
					.map(product -> {
						financeCompanyRepository.findByCode(product.getCompanyNumber()).ifPresent(consumer -> {
							product.setFinanceCompany(consumer);
						});
