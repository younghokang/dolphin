...
public class FinanceCompany {
	...
	public static FinanceCompany from(Company c) {
		FinanceCompany fc = new FinanceCompany();
		fc.setFinanceGroup(c.getFinanceGroup());
		fc.setDisclosureMonth(c.getDisclosureMonth());
		fc.setCode(c.getFinanceCompanyNumber());
		fc.setName(c.getKoreanFinanceCompanyName());
		fc.setDisclosureChargeman(c.getDisclosureChargeMan());
		fc.setWebSite(c.getHomepageUrl());
		fc.setTel(c.getCallCenterTel());
		fc.getOptions().addAll(c.getCompanyOptions().stream()
				.map(option -> {
					FinanceCompanyOption fco = new FinanceCompanyOption();
					fco.setArea(option.getArea());
					fco.setExist(StringUtils.isEmpty(option.getExistsYn()) ? false : option.getExistsYn().equalsIgnoreCase("y") ? true : false);
					return fco;
				})
				.collect(Collectors.toList()));
		fc.setTestState(TestState.READY);
		return fc;
	}
